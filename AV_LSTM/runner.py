...
from AV_LSTM.av_l_dataloader import AVDataloader
from AV_LSTM.trainer import Trainer, setup_logging
from AV_LSTM.av_blstm import AV_bLSTM
from AV_LSTM.a_blstm import A_bLSTM

def av_lstm_runner(mode, phase, dataset_name, asr_flag, plc_loss_rates, mask_types=("gilbert",), gap_durations=(), save_output=False, sample_paths=None):
    ...
    project_name = 'AV_LSTM'
    log_dir = project_log_dir(project_name)
    checkpoint_dir = project_checkpoint_dir(project_name)
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(checkpoint_dir, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    if mode == 'av':
        model_name = f"av_lstm{'_asr' if asr_flag else ''}_bursty({dataset_name})"
        model = AV_bLSTM(asr_loss=asr_flag, bi_dir=bi_dir_flag,
                         hidden_size=hidden_size, num_layers=num_layers)
    elif mode == 'a':
        model_name = f"a_lstm{'_asr' if asr_flag else ''}_bursty({dataset_name})"
        model = A_bLSTM(asr_loss=asr_flag, bi_dir=bi_dir_flag,
                         hidden_size=hidden_size, num_layers=num_layers)
...
if __name__ == "__main__":
    ...
    av_lstm_runner(mode=args.mode, phase=args.phase,
                   dataset_name=args.dataset,
                   asr_flag=args.asr, mask_types=args.mask_types,
                   gap_durations=args.gap_durations, plc_loss_rates=args.plc_loss_rates,
