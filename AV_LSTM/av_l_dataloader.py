...
from AV_LSTM.av_l_dataset import AVDataset
...
class AVDataloader:
    ...
    def train_dataloader(self):
        train_dataset = AVDataset(self.train_files, self.mode, online_loss_bounds=(0.3, 0.9))
    ...
    def val_dataloader(self):
        val_dataset = AVDataset(self.val_files, self.mode, online_loss_bounds=(0.3, 0.9), set_seed=SEED)
    ...
    def test_dataloader(self, mask_range=None, mask_type="gilbert", gap_ms=None, seed=SEED):
        if mask_type == "gilbert":
            ...
            test_dataset = AVDataset(
                self.test_files, mode=self.mode, mask_range=str(mask_range), set_seed=seed,
                mask_type="gilbert",
            )
            ...
            test_dataset = AVDataset(
                self.test_files, mode=self.mode, mask_range='rand', set_seed=None,
    ...
    def av_collate_fn(self, batch):
        ...
        if self.mode == 'av':
            masked_specs, motions, mel_specs, phone_indices, texts, masks, paths = zip(*batch)

            spec_len = 300  # target mel length
            upsampled_motions = []
            for motion in motions:
                if np.any(motion != 0):
                    motion = torch.tensor(motion).unsqueeze(0).permute(0, 2, 1)
                    motion = torch.nn.functional.interpolate(motion, size=spec_len, mode='linear', align_corners=False)
                    motion = motion.permute(0, 2, 1).squeeze(0)
                else:
                    motion = torch.zeros(spec_len,
                                         motion.shape[0] * motion.shape[1] if motion.ndim == 3 else motion.shape[-1])
                upsampled_motions.append(motion)

            motions = torch.stack([torch.as_tensor(motion) for motion in upsampled_motions])
            ...
            texts = torch.stack([torch.as_tensor(text) for text in texts])
            phone_indices = torch.stack([torch.as_tensor(phone_index) for phone_index in phone_indices])
            paths = list(paths)

            return masked_specs, motions, mel_specs, phone_indices, texts, masks, paths

        elif self.mode == 'a':
            masked_specs, mel_specs, phone_indices, texts, masks, paths = zip(*batch)
            masked_specs = torch.stack([torch.as_tensor(x) for x in masked_specs])
            mel_specs = torch.stack([torch.as_tensor(x) for x in mel_specs])
            masks = torch.stack([torch.as_tensor(x) for x in masks])
            texts = torch.stack([torch.as_tensor(text) for text in texts])
            phone_indices = torch.stack([torch.as_tensor(phone_index) for phone_index in phone_indices])
            paths = list(paths)

            return masked_specs, mel_specs, phone_indices, texts, masks, paths
