...
from AV_LSTM.losses import CTCLoss, MSELoss
...
from shared.text_processing import PhonemeTokenizer
...
class Trainer:
    ...
    def __init__(self, model, model_name, mode, asr_loss, pesq_loss,
                 train_loader, val_loader=None, freq_emph=False,
    ...
    def _train_epoch(self, epoch, num_epochs):
        ...
        for batch_idx, batch in enumerate(train_iterator):
            ...
            if self.mode == 'a':
                masked_spec, spec, phone, text, mask, path = batch
                ...
                masked_spec, visual_feats, spec, phone, text, mask, path = batch
                ...
                if self.asr_loss:
                    phone = phone.long().to(self.device)
                    rec_spec, pred_phone = self.model(masked_spec, visual_feats)
                    rec_loss = self.criterion(rec_spec, spec, mask=False)
                    ctc_loss_val = self.ctc_loss(pred_phone, phone)
    ...
    def _validate_epoch(self, epoch, num_epochs):
        ...
        with (torch.no_grad()):
            for batch_idx, batch in enumerate(val_iterator):
                ...
                if batch_idx == 0: #batch_idx % 100 == 0:
                    metrics = calculate_batch_metrics(
                        original_batch=spec.detach().cpu(),
                        reconstructed_batch=rec_spec.detach().cpu(),
                        phones=phone.detach().cpu().numpy() if self.asr_loss else None,
                        pred_phones=pred_phone.detach().cpu().numpy() if self.asr_loss else None,
                        texts=text,
                        path=path,
                        mask=mask,
    ...
    def _log_results(self, epoch, num_samples=2):
            ...
            with torch.no_grad():
                if self.asr_loss:
                    ...
                    self.logger.info(f"target transcriptions : "
                                     f"{self.tokenizer.decode_index_batch(phone[:num_samples].cpu().numpy())}")
                    # Convert logits to indices
                    pred_indices = torch.argmax(pred_phone[:num_samples], dim=-1).cpu().numpy()
        ...
        for i in range(num_samples):
            if self.vocoder is None:
                original_audio = torch_mel_to_audio(spec[i].cpu())
                masked_audio = torch_mel_to_audio(masked_spec[i].cpu())
                reconstructed_audio = torch_mel_to_audio(rec_spec[i].cpu())
            else:
                original_audio = (spec[i])
                masked_audio = mel_to_audio_hifigan(masked_spec[i], self.vocoder)
    ...
    def evaluate(self, test_loader, loss_rate=None, mask_type="gilbert", gap_ms=None,
        ...
        for batch_idx, batch in enumerate(tqdm(test_loader, desc=f"Testing {condition_name}", unit="batch")):
            ...
            phone_accum.append(phone) if self.asr_loss else None
            pred_phone_accum.append(pred_phone.detach().cpu()) if self.asr_loss else None
            ...
            if flush:
                ...
                phones_cat = torch.cat(phone_accum, dim=0) if self.asr_loss else None
                pred_phones_cat = torch.cat(pred_phone_accum, dim=0) if self.asr_loss else None
                ...
                model_metrics = calculate_batch_metrics(
                    ...
                    phones=phones_cat, pred_phones=pred_phones_cat,
                    tokenizer=self.tokenizer if pred_phones_cat is not None else None,
                    **common)
                ...
                phone_accum.clear(); pred_phone_accum.clear()
