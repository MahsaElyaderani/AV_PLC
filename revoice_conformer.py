...
from stable_diffusion.models.plc.resnet import ResNetModel
from stable_diffusion.models.plc.av_l_dataloader import AVDataloader
from stable_diffusion.models.plc.trainer import Trainer, setup_logging
...
class Audio_Decoder(nn.Module):
    ...
    def __init__(self, mel_emb=2*80, mel_dim=80, hidden_size=512,
...
class AV_ReVoice(nn.Module):
    def __init__(self, l2s_loss=False, hidden_size=256,
        ...
        if self.l2s_loss:
            ...
            video_checkpoint_path = '/home/ai/Projects/Mahsa/sources/stable_diffusion/models/l2s/checkpoints/svts(grid)/best_model.pt'
            ...
            self.audio_dec = Audio_Decoder(mel_emb=2*80)
        else:
            self.audio_dec = Audio_Decoder(mel_emb=80)
        #audio_checkpoint_path = '/home/ai/Projects/Mahsa/sources/stable_diffusion/models/checkpoints/a_lstm(skw_mask)_pesq(voxceleb2)/best_model.pt'
    ...
    def forward(self, dec_input, enc_input, spk_emb):
        ...
        if self.l2s_loss:
            x = self.video_enc(enc_input, spk_emb) # [b, d_a=80, t_a=200]
            y = torch.cat((dec_input.permute(0, 2, 1), x.permute(0, 2, 1)),-1) #[b, t_a=200, 2*d_a=160]
            y = self.audio_dec(y.permute(0, 2, 1))
...
if __name__ == "__main__":
    ...
    for dataset_name in dataset_names:
        for l2s_flag in l2s_flags:
                model_name = f"synth_plc_{'_asr' if l2s_flag else ''}{'_pesq' if pesq_flag else ''}({dataset_name})"
