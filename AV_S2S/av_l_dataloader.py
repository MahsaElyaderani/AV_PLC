...
from AV_S2S.av_l_dataset import AVDataset
...
class AVDataloader:
    ...
    def train_dataloader(self):
        train_dataset = AVDataset(base_path=self.train_files,
                                  mode=self.mode, online_loss_bounds=(0.3, 0.9))
    ...
    def val_dataloader(self):
        val_dataset = AVDataset(base_path=self.val_files,
                                mode=self.mode, online_loss_bounds=(0.3, 0.9), set_seed=SEED)
    ...
    def av_collate_fn(self, batch):
        if self.mode == 'v':
            frames, texts, paths = zip(*batch)

            # processed_frames = []
            # target_len = 75
            #
            # for frame in frames:  # frame shape: [T, C, H, W]
            #     t, c, h, w = frame.shape
            #     frame = frame.permute(1, 2, 3, 0).reshape(-1, t).unsqueeze(0)  # [1, c*h*w, T]
            #
            #     if t != target_len:
            #         frame = torch.nn.functional.interpolate(frame, size=target_len,
            #                                             mode='linear', align_corners=False)
            #     new_t = frame.shape[-1]  # should now be 75
            #     frame = frame.squeeze(0).view(c, h, w, new_t).permute(3, 0, 1, 2)  # [75, C, H, W]
            #     processed_frames.append(frame)

            #frames = torch.stack(processed_frames)  # [B, 75, C, H, W]
            frames = torch.stack([torch.as_tensor(frame) for frame in frames])
            texts = torch.stack([torch.as_tensor(text) for text in texts])
            paths = list(paths)

            return frames, texts, paths


        elif self.mode == 'av':

            masked_specs, motions, mel_specs, texts, masks, paths = zip(*batch)
            video_len = 75  # target mel length
            upsampled_motions = []
            for motion in motions:
                if np.any(motion != 0):
                    motion = torch.tensor(motion).unsqueeze(0)
                    motion = motion.permute(0, 2, 1)  # (1, F*C, T)
                    motion = torch.nn.functional.interpolate(motion, size=video_len,
                                                             mode='linear',
                                                             align_corners=False)
                    motion = motion.permute(0, 2, 1).squeeze(0)  # (spec_len, F*C)
            ...
            return masked_specs, motions, mel_specs, texts, masks, paths

        elif self.mode == 'a':
            masked_specs, mel_specs, texts, masks, paths = zip(*batch)
            ...
            return masked_specs, mel_specs, texts, masks, paths
