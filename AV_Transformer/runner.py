...
from AV_Transformer.av_dataloader import AVDataloader
from AV_Transformer.trainer import Trainer, setup_logging
from AV_Transformer.av_transformer import EncoderDecoder as AVEncoderDecoder
from AV_Transformer.a_transformer import EncoderDecoder as AudioEncoderDecoder

def av_transformer_runner(mode, phase, dataset_name, plc_loss_rates, mask_types=("gilbert",), gap_durations=(), save_output=False, sample_paths=None):
    ...
    log_dir = project_log_dir('AV_Transformer')
    checkpoint_dir = project_checkpoint_dir('AV_Transformer')
    os.makedirs(log_dir, exist_ok=True)
    os.makedirs(checkpoint_dir, exist_ok=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    if mode == "a":
        model_name = f"a_transformer_bursty({dataset_name})"
        logger = setup_logging(model_name, log_dir)
        model = AudioEncoderDecoder()
    elif mode == "av":
        model_name = f"av_transformer_bursty({dataset_name})"
        logger = setup_logging(model_name, log_dir)
        model = AVEncoderDecoder()
    ...
    vocoder_path = None #'/home/ai/Projects/Mahsa/sources/stable_diffusion/dataset/hifigan/checkpoints/seg_len_4096/model-best.pt'
    ...
    if phase == 'test':
        ...
        if "gilbert" in mask_types:
            for plc_loss_rate in plc_loss_rates:
                test_loader = av_dataloader.test_dataloader(mask_type="gilbert",
                                                            mask_range=plc_loss_rate, seed=SEED)
...
if __name__ == "__main__":
    ...
    av_transformer_runner(mode=args.mode, phase=args.phase,
                   dataset_name=args.dataset,
                   plc_loss_rates=args.plc_loss_rates,
                   mask_types=args.mask_types, gap_durations=args.gap_durations,
