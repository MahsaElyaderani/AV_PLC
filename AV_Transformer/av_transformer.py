...
class Encoder(nn.Module):
    def __init__(self, audio_dim=spec_dim, video_dim=768,
        ...
        self.video_emb = nn.Sequential(nn.Linear(video_dim, hidden_size),
                                       nn.ELU(),
                                       nn.Linear(hidden_size, hidden_size),
                                       nn.ELU())
        ...
        self.register_parameter('audio_mod_enc', nn.Parameter(torch.rand(hidden_size)))
        self.register_parameter('video_mod_enc', nn.Parameter(torch.rand(hidden_size)))

        self.blocks = nn.ModuleList([TransformerBlock(hidden_size, num_heads,
                                                      is_decoder=False, masking=False)
                                     for _ in range(num_layers)])

    def forward(self,  audio, video, padding_mask=None):
        ...
        a_emb = a_emb + pos_emb_a + self.audio_mod_enc

        v_emb = self.video_emb(video)
        bv, lv, fv = v_emb.shape
        seq_indx_v = torch.arange(lv, device=v_emb.device)
        pos_emb_v = self.pos_emb(seq_indx_v).reshape(1, lv, fv).expand(bv, lv, fv)
        v_emb = v_emb + pos_emb_v + self.video_mod_enc

        embs = torch.cat([a_emb, v_emb], dim=1)
...
class Decoder(nn.Module):
    def __init__(self, audio_dim=spec_dim, hidden_size=512, num_layers=7, num_heads=8):
...
class EncoderDecoder(nn.Module):
    def __init__(self, audio_dim=spec_dim, video_dim=768, hidden_size=512, num_layers=(6, 7), num_heads=8):
        super(EncoderDecoder, self).__init__()

        self.encoder = Encoder(audio_dim=audio_dim, video_dim=video_dim, hidden_size=hidden_size,
    ...
    def forward(self, audio, video):
        ...
        encoded_seq = self.encoder(audio=audio.permute(0,2,1),
                                   video=video,
                                   padding_mask=input_key_mask)

        decoded_seq = self.decoder(audio=audio.permute(0,2,1),
